/* * MAPAS - BSCinfos * ------------------------------------------------------------ * Esta tela NÃO substitui nenhuma função do app.js. * Ela usa: * - ROTACAO_MAPAS já existente no app.js * - historico_bruto.csv já carregado pelo app.js * - _estatisticasPorSetCache, quando o app.js disponibilizar * - rosters.json/ROSTERS_AUTOMATICOS, quando disponível * * As estatísticas são contabilizadas por SET. * Um SET é formado por 2 ou 3 games do mesmo confronto, * no mesmo modo/mapa e com a mesma composição. */
...
(function () {
  ...
  function obterRegiaoAtualMapas() {
    return String(
      typeof _REGIAO !== "undefined" ? _REGIAO : window.REGIAO_ATUAL || "SA"
    ).toUpperCase();
  }
  ...
  function obterNomeTimeAutomatico(idTime) {
    ...
    if (rosters && typeof rosters === "object") {
      for (const item of Object.values(rosters)) {
        if (item && String(item.id_time || "") === id && item.nome_time) {
  ...
  function obterLinhasFonte() {
    /* * Preferimos os dados já condensados por SET quando o app.js * disponibilizar o cache. Isso evita contar os 3 jogadores * ou os games individualmente. */
    try {
      const cache = window._estatisticasPorSetCache;
      if (cache && Array.isArray(cache.linhas) && cache.linhas.length) {
    ...
    if (!bruto.length) {
      try {
        if (
          typeof dadosFiltrados !== "undefined" &&
  ...
  function agruparGames(bruto) {
    ...
    grupos.forEach((linhas) => {
      ...
      const vencedor = numero(ladoA[0].win) === 1 ? teamA : teamB;
  ...
  function condensarHistoricoEmSets(bruto) {
    ...
    /* * Um SET: * - mesmo confronto; * - mesmo modo; * - mesmo mapa; * - mesma composição dos dois lados; * - máximo de 3 games; * - termina em 2 vitórias de um lado ou após o 3º game. */
    games.forEach((game) => {
      const chave = [
    ...
    /* * Só consideramos SET completo quando terminou 2-0, 2-1 ou 1-2. * Assim uma partida isolada não vira artificialmente um SET. */
    return sets
      .filter((set) => {
        const n = set.games.length;
        return n >= 2 && n <= 3 && (set.winsA === 2 || set.winsB === 2);
      })
      .map((set, index) => ({
        ...set,
        id_set: `${set.modo}|${set.mapa}|${set.teamA}|${set.teamB}|${index}`,
        vencedor: set.winsA > set.winsB ? set.teamA : set.teamB,
      }));
  }
  ...
  function obterSetsParaMapa(modo, mapa) {
    ...
    return condensarHistoricoEmSets(bruto).filter(
      (set) =>
  ...
  function obterTimesCadastrados() {
    ...
    const registrar = (item, regiaoInformada) => {
      ...
      const regiao = String(
        item.regiao || regiaoInformada || "SA"
      ).toUpperCase();
      if (!id || /^UNK/i.test(id) || /desconhecid|unknown|unknow/i.test(nome))
        return;
    ...
    const percorrer = (obj, regiaoInformada) => {
      ...
      if (Array.isArray(obj)) {
        obj.forEach((x) => percorrer(x, regiaoInformada));
        return;
      }
      if (obj.id_time || obj.id) registrar(obj, regiaoInformada);
      Object.keys(obj).forEach((chave) => {
        const valor = obj[chave];
        const novaRegiao = /^(SA|NA|EA|EMEA|ALL)$/i.test(chave)
          ? chave.toUpperCase()
          : regiaoInformada;
    ...
    try {
      const cfg =
        typeof CONFIGURACAO_MANUAL_TIMES !== "undefined"
          ? CONFIGURACAO_MANUAL_TIMES
          : window.CONFIGURACAO_MANUAL_TIMES;
      percorrer(cfg, null);
    } catch (_) {}
    return mapa;
  }
  ...
  function construirEstatisticas(modo, mapa) {
    ...
    const registrarSinergia = (a, b, venceu) => {
      ...
      const x = sinergias.get(chave);
      x.games++;
      venceu ? x.wins++ : x.losses++;
    };

    const registrarComp = (time, comp, venceu) => {
      if (!time || !Array.isArray(comp) || comp.length !== 3) return;
      const trio = [
        ...new Set(comp.map((x) => String(x).toUpperCase())),
      ].sort();
    ...
    sets.forEach((set) => {
      ...
      games.forEach((game) => {
        const a = registrarTime(game.teamA, game.nomeA),
          b = registrarTime(game.teamB, game.nomeB);
        ...
        for (let i = 0; i < ca.length; i++)
          for (let j = i + 1; j < ca.length; j++)
            registrarSinergia(ca[i], ca[j], venceuA);
        for (let i = 0; i < cb.length; i++)
          for (let j = i + 1; j < cb.length; j++)
            registrarSinergia(cb[i], cb[j], venceuB);
        registrarComp(a, ca, venceuA);
        registrarComp(b, cb, venceuB);
      });
    });

    const timesArray = Array.from(times.values())
      .filter((t) => t.games > 0)
      .map((t) => ({ ...t, winRate: (t.wins / t.games) * 100 }));
    const sinergiasArray = Array.from(sinergias.values())
      .filter((x) => x.games > 0)
      .map((x) => ({ ...x, winRate: (x.wins / x.games) * 100 }));
  ...
  function criarTabelaTimes(lista) {
    ...
    return `<div class="mapas-table-wrap"><table class="excel-table mapas-table"><thead><tr><th>TIME</th><th>WINRATE%</th><th>WINS</th><th>LOSSES</th><th>GAMES</th></tr></thead><tbody> ${lista .map( (t) => `<tr><td style="text-align:left;"><div class="mapas-team-cell"><img src="${escapeHtml( teamLogoUrlSafe(t.id) )}" onerror="this.onerror=null;this.src='${escapeHtml( teamLogoFallbackSafe(t.id) )}'"><span>${escapeHtml( t.nome )}</span></div></td><td class="winrate-cell">${t.winRate.toFixed( 1 )}%</td><td>${t.wins}</td><td>${t.losses}</td><td>${ t.games }</td></tr>` ) .join("")} </tbody></table></div>`;
  }

  function criarTabelaSinergias(lista) {
    if (!lista.length)
      return `<div class="mapas-empty">Sem sinergias suficientes.</div>`;
    return `<div class="mapas-table-wrap"><table class="excel-table mapas-table"><thead><tr><th>SINERGIA</th><th>WINRATE%</th><th>WINS</th><th>LOSSES</th><th>GAMES</th></tr></thead><tbody> ${lista .map( (s) => `<tr><td style="text-align:left;font-weight:900;"><div class="mapas-synergy-cell">${s.brawlers .map( (b) => `<span class="mapas-brawler-pill"><img src="brawlers/pins/${escapeHtml( formatImgSafe(b) )}.png" onerror="this.onerror=null;this.src='brawlers/${escapeHtml( formatImgSafe(b) )}.png'">${escapeHtml(b)}</span>` ) .join( "" )}</div></td><td class="winrate-cell">${s.winRate.toFixed( 1 )}%</td><td>${s.wins}</td><td>${s.losses}</td><td>${ s.games }</td></tr>` ) .join("")} </tbody></table></div>`;
  }

  function criarTabelaComps(lista) {
    if (!lista.length)
      return `<div class="mapas-empty">Sem comps suficientes.</div>`;
    return `<div class="mapas-table-wrap"><table class="excel-table mapas-table"><thead><tr><th>TIME</th><th>COMP</th><th>WINRATE%</th><th>WINS</th><th>LOSSES</th><th>GAMES</th></tr></thead><tbody> ${lista .map( (c) => `<tr><td style="text-align:left;font-weight:900;">${escapeHtml( c.teamName )}</td><td style="text-align:left;">${c.comp .map( (b) => `<span class="mapas-brawler-pill"><img src="brawlers/pins/${escapeHtml( formatImgSafe(b) )}.png" onerror="this.onerror=null;this.src='brawlers/${escapeHtml( formatImgSafe(b) )}.png'">${escapeHtml(b)}</span>` ) .join("")}</td><td class="winrate-cell">${c.winRate.toFixed( 1 )}%</td><td>${c.wins}</td><td>${c.losses}</td><td>${ c.games }</td></tr>` ) .join("")} </tbody></table></div>`;
  }
  ...
  function renderizarSidebarMapas() {
    ...
    if (!modos.length) {
      sidebar.innerHTML = ` <div class="mapas-empty"> Nenhuma rotação de mapas encontrada no app.js. </div> `;
      return;
    }
    ...
    modos.forEach((modo, modoIndex) => {
      html += ` <div class="mapas-modo-title"> <img src="element/modes/${escapeHtml( formatImgSafe(modo) )}.png" onerror="this.style.display='none'"> <span>${escapeHtml(modo)}</span> </div> `;

      rotacao[modo].forEach((mapa, mapaIndex) => {
        const id = `mapa-${modoIndex}-${mapaIndex}`;

        html += ` <button type="button" id="${id}" class="mapas-sidebar-item" data-modo="${escapeHtml(modo)}" data-mapa="${escapeHtml(mapa)}" > <img src="element/maps/${escapeHtml( formatImgSafe(mapa) )}.png" onerror="this.style.display='none'"> <span>${escapeHtml(mapa)}</span> </button> `;
      });
    });
    ...
    if (!mapaSelecionado) {
      ...
      window.selecionarMapaAnalise(mapaSelecionado.modo, mapaSelecionado.mapa);
    }
  }
  ...
  function renderizarDetalhesMapa(modo, mapa) {
    ...
    const pioresTimes = [...dados.times]
      .sort(
        (a, b) => a.winRate - b.winRate || a.wins - b.wins || b.games - a.games
      )
      .slice(0, 5);
    ...
    painel.innerHTML = ` <div class="mapas-header"> <div> <div class="mapas-mode-label">${escapeHtml(modo)}</div> <h2>${escapeHtml(mapa)}</h2> <div class="mapas-subtitle"> ${dados.sets.reduce( (total, set) => total + (Array.isArray(set.games) ? set.games.length : 0), 0 )} GAMES analisados </div> </div> <img class="mapas-main-image" src="element/maps/${escapeHtml(formatImgSafe(mapa))}.png" onerror="this.style.display='none'" > </div> <section class="mapas-section"> <div class="mapas-section-title"> <span>🏆 MELHORES TIMES</span> <small>Contado por GAME</small> </div> ${criarTabelaTimes(melhoresTimes)} </section> <section class="mapas-section"> <div class="mapas-section-title"> <span>📉 PIORES TIMES</span> <small>Top 5 com menor WinRate</small> </div> ${criarTabelaTimes(pioresTimes)} </section> <section class="mapas-section"> <div class="mapas-section-title"> <span>🤝 MELHORES SINERGIAS</span> <small>Par de brawlers do mesmo time, contado por GAME</small> </div> ${criarTabelaSinergias(melhoresSinergias)} </section> <section class="mapas-section"> <div class="mapas-section-title"> <span>🔥 MELHORES COMPS</span> <small>Trio do mesmo time, contado por GAME</small> </div> ${criarTabelaComps(melhoresComps)} </section> `;
  }
  ...
  function instalarIntegracaoComApp() {
    /* * processarDadosGlobais já existe no app.js. * Envolvemos a função sem apagar a original. */
    if (
  ...
  document.addEventListener("DOMContentLoaded", () => {
    ...
    /* * O CSV é carregado de forma assíncrona pelo app.js. * Fazemos pequenas verificações até os dados aparecerem, * sem alterar o fluxo original do aplicativo. */
    let tentativas = 0;
    ...
    const timer = setInterval(() => {
      ...
      try {
        if (typeof dadosBrutos !== "undefined") {
          quantidade = Array.isArray(dadosBrutos) ? dadosBrutos.length : 0;
        }
      } catch (_) {}
  ...
  /* * Se o usuário entrar na tela MAPAS depois do carregamento, * ela é renderizada novamente. */
  /* * NAVEGAÇÃO DOS MAPAS * -------------------------------------------------------- * Não usamos mais onclick inline nos botões. * O app.js possui outros listeners de clique e, dependendo * da ordem em que os arquivos são carregados, o clique podia * não chegar à função de seleção. * * Este listener usa CAPTURE e fica no document para garantir * que qualquer mapa clicado seja selecionado, mesmo depois * que a sidebar seja reconstruída dinamicamente. */
  document.addEventListener(
    "click",
    (event) => {
      const alvo = event.target.closest?.("#sidebar-mapas .mapas-sidebar-item");

      if (alvo) {
        event.preventDefault();
        event.stopPropagation();

        const modo = alvo.getAttribute("data-modo") || "";
        const mapa = alvo.getAttribute("data-mapa") || "";

        if (
          modo &&
          mapa &&
          typeof window.selecionarMapaAnalise === "function"
        ) {
      ...
      const menuMapas = event.target.closest?.(
        "a[onclick*=\"mudarTela('mapas')\"]"
      );
  ...
  /* * Também suporta chamadas programáticas/teclado nos botões * criados dinamicamente pela sidebar. */
  document.addEventListener(
    "keydown",
    (event) => {
