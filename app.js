...
function mesclarObjetosTimes(destino, origem) {
    for (let regiao in origem) {
        if (!destino[regiao]) destino[regiao] = {};
        for (let tier in origem[regiao]) {
            if (!destino[regiao][tier]) destino[regiao][tier] = [];
            origem[regiao][tier].forEach(time => {
                if (!destino[regiao][tier].find(t => t.id_time === time.id_time)) {
                    destino[regiao][tier].push(JSON.parse(JSON.stringify(time)));
                }
            });
        }
    }
...
function atualizarRostersAtuais() {
    ...
    if (ano !== 'todos' && mes !== 'todos' && ROSTERS_POR_DATA[ano] && ROSTERS_POR_DATA[ano][mes]) {
        ...
        // Agrupa os times de todos os meses disponíveis se nenhum mês for selecionado
        for (let a in ROSTERS_POR_DATA) {
            for (let m in ROSTERS_POR_DATA[a]) {
                mesclarObjetosTimes(CONFIGURACAO_MANUAL_TIMES, ROSTERS_POR_DATA[a][m]);
            }
        }
    }
...
function mesclarTimesSalvosEmRostersPorData() {
    ...
    if (ano !== 'todos' && mes !== 'todos' && ROSTERS_POR_DATA[ano] && ROSTERS_POR_DATA[ano][mes]) {
        ...
        for (let a in ROSTERS_POR_DATA) {
            for (let m in ROSTERS_POR_DATA[a]) {
                alvos.push(ROSTERS_POR_DATA[a][m]);
            }
        }
    }
...
function renderizarDetalheScrim(scrim) {
    ...
    detalhe.innerHTML = `
        ...
        <div id="scrim-round-detalhe" style="margin-top:20px;"></div>
    `;

    if (scrim.roundsMD3.length > 0) window.selecionarRoundMD3(0, detalhe.querySelector('.scrim-round-btn'));
}
