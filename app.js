...
window.registrarTimeCustom = function(idAntigo) {
    ...
    if (exportBox) {
        ...
        exportBox.innerHTML = `
            <div style="background:rgba(176,0,255,0.07); border:1px dashed var(--accent-purple); padding:15px; border-radius:8px; margin-top:10px;">
                <p style="color:#fff; font-size:12px; margin-bottom:10px; font-weight:bold;">Adicione ao dicionário do Python:</p>
                <pre style="background:var(--bg-geral); color:var(--winrate-color); padding:10px; border-radius:6px; font-size:11px; white-space:pre-wrap; word-break:break-all;">${linhasPy}</pre>
            </div>`;
    }
    
    renderizarSidebarTimes();
    timeSelecionado = novoTime;
    renderizarDetalhesTime(novoTime);
