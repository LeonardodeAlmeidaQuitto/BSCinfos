...
// ========================================================
let ROSTERS_POR_DATA = { PADRAO: {} };
let ROSTERS_JSON_CARREGADO = false;

async function carregarRostersAutomaticos() {
    try {
        const resposta = await fetch("rosters.json", { cache: "no-store" });
        if (!resposta.ok) throw new Error(`HTTP ${resposta.status}`);
        const gerados = await resposta.json();
        if (gerados && typeof gerados === "object") {
            ROSTERS_POR_DATA = gerados;
            ROSTERS_JSON_CARREGADO = true;
            console.log("Rosters automáticos carregados.");
        }
    } catch (erro) {
        console.error("Não foi possível carregar rosters.json:", erro);
        ROSTERS_POR_DATA = { PADRAO: {} };
    }
}
